...
### Dynamic groups in Azure AD and Exchange Online

Just to make things clear, the concept of dynamic groups already exist in the Microsoft 365 cloud platform:

__Dynamic Security Groups__ can be created in Azure AD. This kind of group can not however be mail-enabled currently.

__Dynamics Distribution Lists__ in Exchange Online uses rules and queries to determine members. These are executed when an email is sent to the list and there is no easy way for end-users to see who will receive the message.
